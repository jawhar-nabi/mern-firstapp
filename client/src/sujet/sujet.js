import React from 'react';
import './sujet.css';
import {Dimmer, Loader, Form,Icon,  Item, Pagination } from 'semantic-ui-react'
import axios from 'axios';

export class sujet extends React.Component  {
  state ={valeur:'', open: false,
    activePage: 1
  };

  constructor(props){
    super(props);
    this.state={
      sujets:[]
    }
  }

  componentDidMount(){
    axios.get('http://localhost:3000/api/sujets/')
      .then(res => this.setState({ sujets: res.data }))
  }
  handlePaginationChange = (e, { activePage }) => {this.setState({ activePage });     window.scrollTo(0, 0); }

  // handleclick = (id) => {
  //   console.log('/sujet/:id',id){}
  // }
  render(){
    let {
      activePage=1
    } = this.state

    const {valeur} = this.state;
    var {sujets} = this.state;
    var deb = activePage *5   ;
    var fin = 1;
    return(
    <div className="sujets">
      <Form >
        {this.state.sujets ?

      <div>
      <h2 className='namediv'>Subjects</h2> <h5 className='namediv'>page : {activePage } / {fin=Math.ceil(sujets.length/5)} </h5>

         {sujets.slice(deb-5,deb).map(item =>( 
      <Item.Group className="subjects">

        <Item >
        {/* <h6 style={{display:'none'}}> {fin= Math.ceil(sujets.length/5)  } </h6> */}
        {/* <h6>size : { fin= Math.ceil(sujets.length/5)  } activepage : {activePage} deb:{deb} </h6> */}

          <Item.Image  src='data:image/png;base64,' /> 
          <Item.Content key={item.id} className='name-list'>
            <Item.Header className='header1' as='a' value={valeur} onClick={() => setTimeout( () =>{this.props.history.push('/affich-sujet/'+item._id)}, 100)}><b> {item.title} </b> </Item.Header>
            <p className='sujettext'>{item.text}</p><br/>
            <Item.Description><i>Written by :</i> <a href='' onClick={() => this.props.history.push('/account/'+item.author._id)} > {item.author.firstname} {item.author.lastname}</a> </Item.Description>
            <Icon  name='comment' className='b' /> <h6 className='b' style={{color:'gray'}}>Replies : {item.comments.length}</h6>
            <Item.Content content={item.date} className='name-list'  />
          </Item.Content>
        </Item>
      </Item.Group>
      ))}
      <div className='text-center'>
          <Pagination     pointing
    secondary
            activePage={activePage}
            onPageChange={this.handlePaginationChange}
            totalPages={fin}
          />
          </div>
        </div> : <Dimmer active >
         <Loader inverted/>
      </Dimmer>
      
      }
      </Form>
    </div>
  )
  }
}
